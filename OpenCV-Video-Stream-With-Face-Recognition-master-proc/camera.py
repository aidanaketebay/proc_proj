        ...
        for (top, right, bottom, left), name in zip(self.face_locations, self.face_names):
            ...
            if name != "Unknown":
                ...
                if not (name in self.last) or self.last[name] + datetime.timedelta(seconds=8) < now:
                    ...
                    sql = "select time_out from attend where id ="+ name+" order by id desc"
                    cur.execute(sql)
                    rr = ""
                    rr2 = ""
                    for row in cur.fetchall() :
                        rr = row[-1]
                    if rr!='':
                        cur.execute("INSERT INTO `attend`(`id`, `time_in`, `time_out`, `date`) VALUES ( "+ name+", current_time() ,'', curdate())")
                    else:
                        sql2 = "select time_in from attend where id = "+ name+" order by id desc"
                        cur.execute(sql2)
